...
public:
	...
	{
		return glm::lookAt(this->position, this->position + this->forward, this->viewUp) * rotateMat;
	}
	// 处理键盘按键后方向移动
	void handleKeyPress(Camera_Movement direction, GLfloat deltaTime)
	{
		...
		case RIGHT:
			...
			break;
		case L_ROTATE:
			rotateMat = glm::rotate(rotateMat, 1.0f * velocity, glm::vec3(0.0f,1.0f,0.0f));
			break;
		case R_ROTATE:
			rotateMat = glm::rotate(rotateMat, -1.0f * velocity, glm::vec3(0.0f,1.0f,0.0f));
			break;
		case U_ROTATE:
			rotateMat = glm::rotate(rotateMat, 1.0f * velocity, glm::vec3(1.0f,0.0f,0.0f));
			break;
		case D_ROTATE:
			rotateMat = glm::rotate(rotateMat, -1.0f * velocity, glm::vec3(1.0f,0.0f,0.0f));
			break;
...
public:
	...
	glm::mat4 rotateMat;//视图的旋转矩阵，用来处理围绕模型的旋转视角
};
