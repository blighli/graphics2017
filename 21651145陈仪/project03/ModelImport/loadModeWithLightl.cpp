...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 设置后面源属性 点光源
		lightAmbientLoc = glGetUniformLocation(shader.programId, "light1.ambient");
		lightDiffuseLoc = glGetUniformLocation(shader.programId, "light1.diffuse");
		lightSpecularLoc = glGetUniformLocation(shader.programId, "light1.specular");
		lightPosLoc = glGetUniformLocation(shader.programId, "light1.position");
		attConstant = glGetUniformLocation(shader.programId, "light1.constant");
		attLinear = glGetUniformLocation(shader.programId, "light1.linear");
		attQuadratic = glGetUniformLocation(shader.programId, "light1.quadratic");
		glUniform3f(lightAmbientLoc, 0.2f, 0.2f, 0.2f);
		glUniform3f(lightDiffuseLoc, 1.0f, 0.0f, 0.0f);
		glUniform3f(lightSpecularLoc, 1.0f, 1.0f, 1.0f);
		glUniform3f(lightPosLoc, lampPos.x, lampPos.y, lampPos.z - 100.0f);
		// 设置衰减系数
		glUniform1f(attConstant, 1.0f);
		glUniform1f(attLinear, 0.09f);
		glUniform1f(attQuadratic, 0.032f);

		glUniform1i(glGetUniformLocation(shader.programId, "lighton"), lighton);
...
{	
	...
	if (keyPressedStatus[GLFW_KEY_LEFT])
		camera.handleKeyPress(L_ROTATE, deltaTime);
	if (keyPressedStatus[GLFW_KEY_RIGHT])
		camera.handleKeyPress(R_ROTATE, deltaTime);
	if (keyPressedStatus[GLFW_KEY_UP])
		camera.handleKeyPress(U_ROTATE, deltaTime);
	if (keyPressedStatus[GLFW_KEY_DOWN])
		camera.handleKeyPress(D_ROTATE, deltaTime);
